...
namespace LMSProject.Areas.Admin.Controllers
{
    ...
    public class AdminCourseController : BaseController
    {
        ...
        public AdminCourseController(AppDbContext db, IUnitOfWork uow, IMapper mapper,
                                     UserManager<ApplicationUser> um) : base(um)
        {
            _db = db; _uow = uow; _mapper = mapper;
        }
                                               ...
                                               string status = "", int page = 1)
        {
            ...
            if (!string.IsNullOrWhiteSpace(grade))
                query = query.Where(c => c.Grade.Name == grade);
            ...
            var vmList = all.Select(c => new CourseListVM
            {
                ...
                MaterialCount = c.CourseMaterials?.Where(m => m.CurrentState == 1).Count() ?? 0,
                ExamCount = examCounts.GetValueOrDefault(c.Id),
                AssignmentCount = c.Assignments?.Where(a => a.CurrentState == 1).Count() ?? 0,
        ...
        public async Task<IActionResult> Edit(EditCourseVM vm)
        {
            ...
            c.status = vm.Status;
            c.Price = vm.Price;
            c.ShowInHomePage = vm.ShowInHomePage;
            c.TermId = vm.TermId;
        ...
        public async Task<IActionResult> Enroll(int courseId, string search = "")
        {
            ...
            var enrolled = await _db.StudentCourses
                .Where(sc => sc.CourseId == courseId).Select(sc => sc.StId).ToListAsync();

            var studentsQ = _db.Students.Include(s => s.Grade).Where(s => s.CurrentState == 1);
            if (!string.IsNullOrWhiteSpace(search))
                studentsQ = studentsQ.Where(s => s.FullName.Contains(search));

            var students = await studentsQ.ToListAsync();

            var vm = new EnrollStudentsVM
            {
                CourseId = courseId,
                CourseName = course.Name,
                InstructorName = course.Instructor?.FullName ?? "—",
                SearchTerm = search,
                Students = students.Select(s => new EnrollStudentItemVM
                {
                    StudentId = s.Id,
                    FullName = s.FullName,
                    Grade = s.Grade?.Name ?? "",
                    IsEnrolled = enrolled.Contains(s.Id)
                }).ToList()
            };
        ...
        public async Task<IActionResult> EnrollStudent(int courseId, int studentId)
        {
            var existing = await _db.StudentCourses
                .FirstOrDefaultAsync(sc => sc.CourseId == courseId && sc.StId == studentId);
        ...
        public async Task<IActionResult> UnenrollStudent(int courseId, int studentId)
        {
            var sc = await _db.StudentCourses
                .FirstOrDefaultAsync(x => x.CourseId == courseId && x.StId == studentId);
            if (sc != null) { _db.StudentCourses.Remove(sc); await _db.SaveChangesAsync(); }
            TempData["Success"] = "Student unenrolled.";
            return RedirectToAction("Enroll", new { courseId });
        }

        // ── JSON for cascade dropdowns ─────────────────────────────────────
        ...
        private async Task<CreateCourseVM> BuildCreateVM(CreateCourseVM vm)
        {
            ...
            vm.SubSubjects = vm.Subjects.Any()
                ? await _db.SubSubjects.Where(ss => ss.CurrentState == 1)
        ...
        {
            var create = await BuildCreateVM(vm);
            // SubSubjects filtered by selected subject
            create.SubSubjects = await _db.SubSubjects
                .Where(ss => ss.SubjectId == vm.SubjId && ss.CurrentState == 1)
                .Select(ss => new SelectDropList { Id = ss.Id, Name = ss.Name }).ToListAsync();
            return (EditCourseVM)create;
        }
    }
}